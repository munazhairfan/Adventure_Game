...
let enemies = ["Skeleton 💀", "Spider 🕷", "Zombie 🧟‍♀️", "Monster 👹", "Mummy 👤"];
let EnemyHealth = 100;
let enemyAttackDamage = 45;
let myHealth = 100;
let myAttackDamage = 50;
let numOfPotion = 3;
let potionHeal = 30;
let healthPotionDropChance = 50; //50 percent
let running = true;
...
Game: while (running) {
    ...
    while (enemyHP > -1) {
        if (myHealth < 30 && myHealth > 0) {
            console.log(`\x1b[33mYou need to drink potion 🚩.\x1b[33m`);
        }
        else if (myHealth <= 0) {
